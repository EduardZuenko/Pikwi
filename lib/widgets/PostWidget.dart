...
import 'package:pikwi/pages/CommentsPage.dart';
...
_PostState({
  ...
  removeUserPost() async{
    postReference.document(ownerId).collection("usersPosts").document(postId).get().then((document){
      if(document.exists){
    ...
    QuerySnapshot querySnapshot = await activityReference.document(ownerId).collection("feedItems").where("postId", isEqualTo: postId).getDocuments();

    querySnapshot.documents.forEach((document) { 
      document.reference.delete();
    });

    QuerySnapshot commetnsQuerySnapshot = await commentsReference.document(postId).collection("comments").getDocuments();

    commetnsQuerySnapshot.documents.forEach((document) { 
      if(document.exists){
        document.reference.delete();
      }
    });
  ...
  removeLike(){
    ...
    if(isNotPostOwner){
      activityReference.document(ownerId).collection("feedItems").document(postId).get().then((document){
        if(document.exists){
          document.reference.delete();
        }
      });
  ...
  addLike(){
    ...
    if(isNotPostOwner){
      activityReference.document(ownerId).collection("feedItems").document(postId).setData({
        "type": "like",
        ...
        Row(
          ...
          children: <Widget>[
            ...
            GestureDetector(
              ...
              child: Icon(Icons.chat_bubble_outline, size: 28.0,
                color: Colors.pink,               
              ),
            )
          ]
